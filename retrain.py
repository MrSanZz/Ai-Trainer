...
def re_train_model(epochs=5):
    """
    Fungsi untuk re-training model dari file ai_model.json menggunakan dataset eksternal.
    Mirip dengan konsep epoch dan loss di PyTorch, namun untuk model n-gram sederhana.
    Loss dihitung secara pseudo dengan negative log likelihood dari probabilitas bigram.
    """
    from datasets import load_dataset
    chatbot = NeuralChat()  # Memuat model yang sudah ada
    ...
    chatbot.safe_save()
    logging.info("Re-training finished. Model saved successfully.")
...
def main():
    ...
    Tambahan mode 'retrain' disediakan untuk re-training model.
    """
    try:
        epochs = int(input("Enter number of epochs for re-training: "))
    except ValueError:
        epochs = 5
